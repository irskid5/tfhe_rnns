...
pub fn mod_to_precision<T: Signed + AddAssign + std::ops::Shl<i32, Output=T> + std::cmp::PartialOrd>(x: T, log_p: i32) -> T {
...
pub fn sgn_with_mod<T: Signed + AddAssign + std::ops::Shl<i32, Output=T> + std::cmp::PartialOrd>(x: T, log_p: i32) -> T {
    let s = mod_to_precision(x, log_p);
    sgn_zero_is_one(s)
}
